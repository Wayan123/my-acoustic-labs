...
import noisereduce as nr  # Tambahkan pustaka noise reduction
    ...
    with sd.InputStream(
        ...
        while True:
            ...
            if silence_count >= required_silence_blocks and len(audio_buffer) > 0:
                ...
                # Noise Reduction
                # print("=== Melakukan noise reduction... ===")
                reduced_audio = nr.reduce_noise(
                    y=full_audio, 
                    sr=sample_rate, 
                    prop_decrease=0.8  # Parameter pengurangan noise (0.8 = pengurangan lebih kuat)
                )

                # Transkripsi audio yang sudah dibersihkan
                # print("=== Mendeteksi akhir percakapan, mulai transkripsi... ===")
                segments, info = model.transcribe(
                    reduced_audio, beam_size=5, language="id"
                )
